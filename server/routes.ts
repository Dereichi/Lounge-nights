...
export async function registerRoutes(
  ...
  // Seed data function - only run if database is available
  async function seedData() {
    try {
      // Only seed if DATABASE_URL is available
      if (!process.env.DATABASE_URL) {
        console.log("DATABASE_URL not set, skipping seed data");
        return;
      }
      ...
      console.log("Database not available, skipping seed data:", error.message);
    }
  }
